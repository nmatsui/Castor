...
+ (UIImageView *)_makeIcon:(CGRect)rect image:(UIImage *)image;
@end
...
+ (UIView *)getRoomCellView:(CGSize)size room:(RoomData *)room portrate:(BOOL)portrate
{
    ...
    UIImageView *icon = [ViewUtil _makeIcon:CGRectMake(20, 5, 30, 30) image:room.roomIcon];
    [v addSubview:icon];
    float w = (portrate) ? size.width - 70 : size.height - 70;
    CGSize s = [room.roomName sizeWithFont:[UIFont systemFontOfSize:GROUP_FONT_SIZE] constrainedToSize:CGSizeMake(w, 1024) lineBreakMode:UILineBreakModeCharacterWrap];
    UILabel *nameLabel = [ViewUtil _makeLabel:CGRectMake(60, 10, w, s.height) text:room.roomName font:[UIFont systemFontOfSize:GROUP_FONT_SIZE]];
...
+ (UIView *)getEntryCellView:(CGSize)size entry:(EntryData *)entry portrate:(BOOL)portrate
{
    ...
    UILabel *indentLabel = [ViewUtil _makeLabel:CGRectMake(10, 10, 40, 16) text:str font:[UIFont systemFontOfSize:INDENT_FONT_SIZE]];
    [v addSubview:indentLabel];
    UIImageView *icon = [ViewUtil _makeIcon:CGRectMake(20+[entry.level intValue]*INDENT_WIDTH, 5, 30, 30) image:entry.participationIcon];
    [v addSubview:icon];
    UILabel *nameLabel = [ViewUtil _makeLabel:CGRectMake(60+[entry.level intValue]*INDENT_WIDTH, 10, 250, 16) text:entry.participationName font:[UIFont boldSystemFontOfSize:ENTRY_FONT_SIZE]];
    [v addSubview:nameLabel];
    float w = (portrate) ? size.width - 100 : size.height - 100;
    if (entry.attachmentType != nil) {
        UILabel *attachmentLabel = [ViewUtil _makeLabel:CGRectMake(w, 10, 100, 16) text:[NSString stringWithFormat:@"<%@ attached>", entry.attachmentType] font:[UIFont systemFontOfSize:ENTRY_FONT_SIZE]];
        [v addSubview:attachmentLabel];
    }
    CGSize s = [entry.content sizeWithFont:[UIFont systemFontOfSize:ENTRY_FONT_SIZE] constrainedToSize:CGSizeMake(w, 1024) lineBreakMode:UILineBreakModeCharacterWrap];
    UILabel *contentLabel = [ViewUtil _makeLabel:CGRectMake(60+[entry.level intValue]*INDENT_WIDTH, 30, w-[entry.level intValue]*INDENT_WIDTH, s.height) text:entry.content font:[UIFont systemFontOfSize:ENTRY_FONT_SIZE]];
...
+ (UIView *)getNextPageCellView:(CGSize)size portrate:(BOOL)portrate
{
    ...
    UILabel *nextLabel = [ViewUtil _makeLabel:CGRectMake(10, 0, w, 40) text:@"<<load next page>>" font:[UIFont systemFontOfSize:ENTRY_FONT_SIZE]];
